...
var PostbirdAlertBox = {
    ...
    getPromptTemplate: function () {
        return '<div class="postbird-box-container">' +
            ...
            '</div>' +
            '<div class="postbird-box-text">' +
            '<input type="text" class="postbird-prompt-input" autofocus="true" style="float: left; width:45%; height:100% ">' +
            '<select class="postbird-prompt-select" style="float: left; width:45%; margin-left: 10%" >'+
    ...
    prompt: function (opt) {
        ...
        var promptSelect = document.getElementsByClassName('postbird-prompt-select');
        promptSelect = promptSelect[promptSelect.length - 1];
        
        var okBtn = document.getElementsByClassName('btn-footer-ok');
        var inputData = promptInput.value;;
        okBtn[okBtn.length - 1].onclick = function () {
            if (opt.onConfirm) {
                opt.onConfirm(promptInput.value+","+promptSelect.value);
            }
            _this.removeBox(box);
        }
        var cancelBtn = document.getElementsByClassName('btn-footer-cancel');
