...
<head>
	...
	<script type="text/javascript">
		...
		//判断是否存在相同的编号
		function existbianhao(){
		
			var bianhao = document.getElementsByName("bianhao")[0] ;
			
			$.ajax({
	             type: "POST",
	             url: "${pageContext.request.contextPath}/existBianHao",
	             data: { 'bianhao' : bianhao.value },
	             dataType: "json",
	             success: function(data){
	             	var datas = eval(data);
	             	
	             	if( "err" == datas[0].flag ){
	             		bianhao.style.borderColor = 'red';
	             		document.getElementById("bianhaotishi").style.display = 'inline';
	             		
	             		mybianhao = 0;
	             	}
	             	
	             	if( "success" == datas[0].flag ){
	             		bianhao.style.borderColor = 'green';
	             		document.getElementById("bianhaotishi").style.display = 'none';
		...
		function test(){
			...
			if(myleng != 1 || mybianhao != 1 || mychepaihao != 1){
				return false ;
			}
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			<div class="col-lg-12">
				...
				<br/>
				<!-- 添加车辆 -->
				<form name="myform" action="${pageContext.request.contextPath}/addXinChe" onsubmit="return test()" method="post">	
					
					<div class="form-group" style="width: 30%; float: left; margin-left: 30px">
						<label>编号<span style="color: red;  padding-top: 10px">*&nbsp;</span><span id="bianhaotishi" style="color: red; display: none;">存在相同编号</span></label>
						<input name="bianhao" class="form-control" onblur="existbianhao()" placeholder="请输入编号" required="required"/>
					</div>					
					...
					<div class="form-group" style="width: 30%; float: left; margin-left: 10%">
						...
						<input name="yunshuleibie" class="form-control" placeholder="请输入运输类别" required="required"/>
					</div>	
					...
					<div class="form-group" style="width: 30%; float: left; margin-left: 30px">
						...
						<input name="lianxiren" class="form-control" placeholder="请输入联系人" required="required"/>
					</div>					
					<div class="form-group" style="width: 30%; float: left; margin-left: 10%">
						...
						<input name="lianxidianhua" class="form-control" onblur="dianhualeng()" placeholder="请输入联系电话" required="required"/>
					</div>	
					...
					<div class="form-group" style="width: 30%; float: left; margin-left: 30px">
						...
						<input id="riqi1" name="shijian" class="form-control"  placeholder="请输入时间" required="required"/>
					</div>					
					<div class="form-group" style="width: 30%; float: left; margin-left: 10%">
					...
					<button type="submit" class="btn btn-primary" style="float: left; margin-left: 30px">添加新车</button>
					<a href="${pageContext.request.contextPath}/xinche.jsp"> <button type="button"  class="btn btn-info" style="float: left; margin-left: 10px">返回</button></a>
					<br/>
				</form>	
				<!-- end -->
